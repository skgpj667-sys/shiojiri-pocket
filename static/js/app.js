...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupEventListeners() {
    ...
    // SNS Sub-Platform filter buttons
    if (snsPlatformFilter) {
      snsPlatformFilter.querySelectorAll('.sns-plat-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const plat = btn.dataset.platform;
          state.selectedPlatform = plat;

          // Update buttons UI
          snsPlatformFilter.querySelectorAll('.sns-plat-btn').forEach(b => {
            const isSel = b.dataset.platform === plat;
            if (isSel) {
              b.className = 'sns-plat-btn shrink-0 flex items-center space-x-1 px-2.5 py-1 rounded-lg font-bold bg-purple-700 text-white text-[11px] shadow-sm transition-all';
            } else {
              b.className = 'sns-plat-btn shrink-0 flex items-center space-x-1 px-2.5 py-1 rounded-lg font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 text-[11px] transition-all';
            }
          });
  ...
  function switchTab(tab) {
    ...
    // Show or hide SNS sub-platform filter
    if (snsPlatformFilter) {
      if (tab === 'sns') {
        snsPlatformFilter.classList.remove('hidden');
      } else {
        snsPlatformFilter.classList.add('hidden');
